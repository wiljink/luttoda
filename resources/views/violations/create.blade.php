...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-gray-800 mt-2">Add Violation — {{ $member->firstname }} {{ $member->lastname }}</h1>
</div>
...
<div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200 max-w-xl">
    ...
    <form action="{{ route('members.violations.store', $member) }}" method="POST" class="space-y-5">
        @csrf

        <div>
            <label class="block text-sm font-semibold text-gray-700 mb-1">Violation Date *</label>
            <input type="date" name="violation_date" value="{{ old('violation_date', now()->format('Y-m-d')) }}" required class="w-full rounded border-gray-300 p-2 border focus:ring focus:ring-blue-200">
        </div>

        <div>
            <label class="block text-sm font-semibold text-gray-700 mb-1">Type / Reason *</label>
            <input type="text" name="type" value="{{ old('type') }}" required class="w-full rounded border-gray-300 p-2 border focus:ring focus:ring-blue-200">
        </div>

        <div>
            <label class="block text-sm font-semibold text-gray-700 mb-1">Notes</label>
            <textarea name="notes" rows="4" class="w-full rounded border-gray-300 p-2 border focus:ring focus:ring-blue-200">{{ old('notes') }}</textarea>
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4" x-data="{ sanction: '{{ old('sanction', '') }}' }">
            <div>
            ...
            <div x-show="sanction === 'suspension'" x-cloak>
                ...
                <input type="date" name="sanction_until" value="{{ old('sanction_until') }}" class="w-full rounded border-gray-300 p-2 border focus:ring focus:ring-blue-200">
            </div>
        </div>
